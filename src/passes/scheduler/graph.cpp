...
namespace graph{
        ...
        for(BasicBlock * direct_pred_bb : predecessors(cur_bb))
        {
                ...
                inst != inst_end; inst++)
            {
                ...
                if(isa<CallInst>(inst))
                {
                    ...
                    else if (Dim3_Inst->getCalledFunction()->getName().str().find("__cudaPushCallConfiguration") != std::string::npos) {
                        addBB(direct_pred_bb);
                        one_bb_split_flag = true;
                        continue;
                    }
    ...
    void GPUFuncNode::SetStream(GlobalVariable * stream, Module & M)
    {
        ...
        Value * cur_stream = builder.CreateLoad(cuStream_t_PtrT,stream_v,"");
    ...
    MemcpyNode::MemcpyNode(CallInst * ci, Function * cf, bool gpu_f = false, bool graph_f = false):FuncNode(ci,cf,gpu_f,graph_f)
    {
        ...
        {
            errs() << "input_arg bitcast: " << *input_arg << "\n";
            ...
            if(LoadInst * ld_inst = dyn_cast<LoadInst>(mid_v))
            {
                ...
                bool flag = false;
                errs() << "input_arg container_v: " << *container_v << "\n";
                ...
                //         flag = true;
                //         errs() << "original_v of " << *input_arg << ": " << *original_v << "\n";
                //         break;
                //     }
                // }
                if (auto alloc_inst = dyn_cast<AllocaInst>(container_v)) {
                    errs() << "input original_v: " << *container_v << "\n";
                    add_input_value(container_v);
                    flag = true;
                }
                else if (CallInst *call_inst = dyn_cast<CallInst>(container_v)) {
                    assert(call_inst->getCalledFunction()->getName() == "malloc" 
                        && "original_v of input_arg must be a malloc call");
                    errs() << "input original_v: " << *container_v << "\n";
                    add_input_value(container_v);
                    flag = true;
                }
            ...
            else if (CallInst *call_inst = dyn_cast<CallInst>(mid_v)) {
                assert(call_inst->getCalledFunction()->getName() == "malloc" 
                    && "original_v of input_arg must be a malloc call");
                errs() << "input original_v: " << *mid_v << "\n";
                add_input_value(mid_v);
            }
            else
            {
                errs()<<"Cannot get the ld_inst of input_arg in MemcpyInst " << *ci << "\n";
                exit(1);
            }
        }
        else
        {   //No bitcast, original value is i8*
            errs() << "input_arg no bitcast: " << *input_arg << "\n";
            if(LoadInst * ld_inst = dyn_cast<LoadInst>(input_arg))
            {
                Value * container_v = ld_inst->getOperand(0);
                bool flag = false;
                if (auto bitcast = dyn_cast<BitCastInst>(container_v)) {
                    container_v = bitcast->getOperand(0);
                }
                errs() << "input_arg container_v: " << *container_v << "\n";
                // for(auto user = container_v->user_begin(), user_end = container_v->user_end();
                //     user != user_end; user++)
            ...
            else if (CallInst *call_inst = dyn_cast<CallInst>(input_arg)) {
                assert(call_inst->getCalledFunction()->getName() == "malloc" 
                    && "original_v of input_arg must be a malloc call");
                errs() << "input original_v: " << *input_arg << "\n";
                add_input_value(input_arg);
            }
            else {
        ...
        {
            errs() << "output_arg bitcast: " << *output_arg << "\n";
            ...
            if(LoadInst * ld_inst = dyn_cast<LoadInst>(mid_v))
            {
                ...
                //     }
                // }
                if (CallInst *call_inst = dyn_cast<CallInst>(container_v)) {
                    assert(call_inst->getCalledFunction()->getName() == "malloc" 
                        && "original_v of output_arg must be a malloc call");
                    errs() << "output original_v: " << *container_v << "\n";
                    add_output_value(container_v);
                    flag = true;
                }
                if(!flag)
                {
                ...
                assert(call_inst->getCalledFunction()->getName() == "malloc" 
                    && "original_v of output_arg must be a malloc call");
                errs() << "output original_v: " << *mid_v << "\n";
                add_output_value(mid_v);
            }
            else
            {
                errs()<<"Cannot get the ld_inst of output_arg in MemcpyInst "<<*ci<<"\n";
                errs() << "output_arg: " << *output_arg << "\n";
                exit(1);
            }
        }
        else
        {   //No bitcast, original value is i8*
            errs() << "output_arg no bitcast: " << *output_arg << "\n";
            if(LoadInst * ld_inst = dyn_cast<LoadInst>(output_arg))
            {
                ...
                //     if(StoreInst * st_inst = dyn_cast<StoreInst>(instr))
                //     {
                //         Value * original_v = st_inst->getOperand(0);
                //         add_output_value(original_v);
                //         flag = true;
            ...
            else if (CallInst *call_inst = dyn_cast<CallInst>(output_arg)) {
                assert(call_inst->getCalledFunction()->getName() == "malloc" 
                    && "original_v of output_arg must be a malloc call");
                errs() << "output original_v: " << *output_arg << "\n";
                add_output_value(output_arg);
            }
            else
            {
                errs()<<"Cannot get the ld_inst of output_arg in MemcpyInst "<<*ci<<"\n";
    ...
    void MemcpyNode::CollectBBs(Module & M)
    {
        ...
        else if (CallInst * call_inst = dyn_cast<CallInst>(first_operand)) {
            assert(call_inst->getCalledFunction()->getName() == "malloc" 
                && "original_v of input_arg must be a malloc call");
            head_inst = call_inst;
        }
        else
        {
            ...
            else if (CallInst * call_inst = dyn_cast<CallInst>(ld_v)) {
                assert(call_inst->getCalledFunction()->getName() == "malloc" 
                    && "original_v of input_arg must be a malloc call");
                head_inst = call_inst;
            }
        ...
        errs()<<"head_inst of " << *callmemcpy_inst << " is " <<*head_inst<<"\n";
        BasicBlock * old_bb = callmemcpy_inst->getParent();
        BasicBlock * cur_bb = nullptr;
        if(head_inst == dyn_cast<Instruction>(old_bb->getFirstInsertionPt()) || head_inst->getParent() != old_bb)
            cur_bb = old_bb;
        else 
    ...
    {
        // Return if we don't want to use async memcpy
        if (useAsyncMemcpy == false) {
            return;
        }
        ...
        size_t arg_num = call_inst->arg_size();
        ...
        Value * tmp_stream_v = dyn_cast<Value>(stream);
        Value * stream_v = builder.CreateLoad(cuStream_t_PtrT,tmp_stream_v,"");
        if(stream_v == nullptr)
        {
    ...
    void DAG::Insert(Node * node)                   //Here we get the func_node in Seq_Graph
    {
        ...
        */
        ...
        std::map<Node*, bool> unique_pred_node_m;
    ...
    void DAG::dump()
    {
        ...
        std::map<Node *, std::vector<Node*>> indegree_map = pred_map;
        std::map<Node *, bool> visited_map;
        
        std::queue<Node*> q;
        q.push(Entry_Node);
        visited_map[Entry_Node] = true;
        ...
        while(!q.empty())
        {
            ...
            for(int i = 0; i < n; i++)
            {
                ...
                q.pop();
                for(auto it = indegree_map.begin(), end = indegree_map.end(); it != end; it++)
                {
                    int target_index = 0;
                ...
                for(auto it = indegree_map.begin(), end = indegree_map.end(); it != end; it++)
                {
                    if(!visited_map[it->first] && it->second.size() == 0) 
                    {
                        q.push(it->first);
                        visited_map[it->first] = true;
                    }
                }
            }
            std::cout<<"********************\n";
    ...
    bool cmp(const std::pair<Node*,size_t> a, const std::pair<Node*,size_t> b)
    {
    ...
    void DAG::StreamDistribute(StreamGraph * StreamG, GlobalVariable ** stream_var_ptrs, Module & M)
    {
        ...
        srand(0);                                                       //To more randomly select a balanced stream

        for(size_t i = 1; i <= n_level; i++)
        {
            std::vector<Node*> nodes = level_nodes_map[i];
            std::map<Node*,bool> setted_flag_map;
            std::map<Node*,size_t> node_original_index_map;
            ...
            for(size_t node_index = 0; node_index < nodes.size(); node_index++)
            {
                ...
                if(cur_node->get_pred_num()==1 && cur_node->get_pred(0) == Entry_Node)
                {
                    ...
                    // Schedule all memcpy node to stream 0
                    if (dynamic_cast<MemcpyNode *>(cur_node) || dynamic_cast<PrefetchNode *>(cur_node))
                    {
                        stream_id = (cur_stream_id) % stream_num;
                    }
                    else
                    {
                        stream_id = (cur_stream_id) % stream_num;
                    ...
                    // dbgs() << "schedule header function to stream " << stream_id << "\n";
                    cur_node->dump_func();
            ...
            for(auto node : nodes)
            {
                ...
                if(stream_end_pred == 0)                //Situation 1. No optimal stream can be selected
                {
                ...
                {
                    //select the pred with least undone succ, which removes redundant synchronizations
            ...
            for(auto node : later_set_nodes)
            {
                ...
                size_t stream_id = cur_stream_id % stream_num;
                // dbgs() << "before: " << cur_stream_id << ", after: " << (cur_stream_id + 1) % stream_num << "\n";
                cur_stream_id = (cur_stream_id + 1) % stream_num;
                StreamG->node_set_stream(node,stream_id,stream_var_ptrs,M);
                StreamG->init_node_undone_succ(node);
                size_t pred_num = node->get_pred_num();
                // dbgs() << "schedule node in later_set_nodes (cur_stream_id = " << cur_stream_id << "): ";
                node->dump_inst();
                //Add EventEdges
                for(size_t j = 0; j < pred_num; j++)
                {
            ...
            // dbgs() << "finish scheduling node in later_set_nodes\n";
        }
    }
    ...
    void StreamGraph::FuncCall_Reorder(BasicBlock * head_bb, BasicBlock * tail_bb, Node * EntryNode)
    {
        ...
        while(flag)
        {
            ...
            for(int i = 0; i < stream_n; i++)
            {
                ...
                {
                    Node * cur_pred = node->get_pred(j);
                    if(visited_map[cur_pred] == false) ready_flag = false;
                    else continue;
                }
                if(ready_flag)
                {
    ...
    void StreamGraph::ChangeLastBBSucc(BasicBlock * pred_bb, Node * node, BasicBlock * tail_bb, BasicBlock ** last_bb)
    {
        ...
        {
            errs() << "pred_br_inst before change: " << *pred_br_inst << "\n";
            ...
            else
            {
                ...
                errs() << "pred_br_inst after change: " << *pred_br_inst << "\n";
            }
        }
        else
        {
    ...
    void StreamGraph::create_Events(Module & M, BasicBlock * head_bb, GlobalVariable** stream_var_ptrs)
    {
        ...
        */
        ...
        for(size_t i = 0; i < EEs.size(); i++)
        {
            ...
            ret_v = builder.CreateLoad(cuEvent_t_PtrT, event_value, "");
            if(ret_v == nullptr)
            {
            ...
            ret_v = nullptr;
            ret_v = builder.CreateLoad(cuStream_t_PtrT, prev_stream_var, "");
            if(ret_v == nullptr)
            {
            ...
            ret_v = builder.CreateLoad(cuStream_t_PtrT,succ_stream_var,"");
            if(ret_v == nullptr)
            {
                errs()<<"Cannot load stream value\n";
                exit(1);
