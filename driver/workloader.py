...
def usage_and_exit():
    ...
    print_flush('  sched_alg: The scheduling algorithm. One of "multiGPU".')
    print()
    print()
    exit(1)
...
def run_benchmark(cmd, wid, active_jobs):
    ...
    # command)
    print_flush('Worker {}: running {}'.format(wid, cmd))
...
if sched_alg not in {'zero', 'single-assignment', 'cg', 'mgb', 'mgb_basic', 'mgb_simple_compute', 'mgb', 'MultiGPU'}:
    usage_and_exit()
if sched_alg in {'mgb_basic', 'mgb_simple_compute', 'mgb', 'MultiGPU'}:
